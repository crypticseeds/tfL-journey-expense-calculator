...
const App: React.FC = () => {
  ...
  const processExpenses = async () => {
    if (files.length === 0 || selectedDates.length === 0) {
      setError('Please upload at least one TfL statement and select your workdays.');
      return;
    }

    setLoadingState({ active: true, message: 'Preparing to analyze files...', progress: 0 });
    setError(null);
    setSummary(null);

    const sessionId = `session-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    
    const traceWrapper = startExpenseTrace(
      sessionId,
      undefined,
      {
    ...
    try {
      await traceWrapper(async (rootSpan: any) => {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          setLoadingState({
            active: true,
            message: `Analyzing file ${i + 1} of ${files.length}: ${file.name}`,
            progress: progress,
          });
          
          const entries = await extractTravelDataFromFile(file, handleProgress, rootSpan);
          allTravelEntries.push(...entries);
        }
        
        setLoadingState({ active: true, message: 'Finalizing summary...', progress: 100 });
        ...
        const filteredEntries = allTravelEntries.filter(entry =>
          entry.date && selectedWorkdayDates.has(entry.date)
        );
        
        const dailyTotalsMap = new Map<string, number>();
        filteredEntries.forEach(entry => {
          dailyTotalsMap.set(entry.date, (dailyTotalsMap.get(entry.date) || 0) + entry.amount);
        });
        ...
        filteredEntries.forEach(entry => {
          ...
          monthlySummariesMap.set(month, (monthlySummariesMap.get(month) || 0) + entry.amount);
        });
  ...
  const canReset = files.length > 0 || selectedDates.length > 0 || summary || error;
      ...
      <div className="max-w-4xl mx-auto">
        <header className="text-center my-12">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">
            TFL Journey <span className="bg-gradient-to-r from-sky-500 to-cyan-500 text-transparent bg-clip-text">Expense Calculator</span>
          </h1>
          <p className="mt-4 text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
            Upload your TfL contactless & Oyster statements to automatically calculate your journey expenses.
          </p>
        </header>
        ...
        <main className="space-y-12">
          <section id="step-1" className="transition-opacity duration-500">
            <h2 className="text-2xl font-bold mb-4 text-slate-700 dark:text-slate-200 flex items-center">
              <span className="flex items-center justify-center w-8 h-8 mr-3 bg-sky-500 text-white rounded-full font-bold">1</span>
              Upload TfL Statements
            </h2>
            <FileUpload onFilesSelected={handleFilesSelected} disabled={loadingState.active} />
          </section>

          {hasFiles && (
            <section id="step-2" className="animate-fade-in space-y-8">
                 <h2 className="text-2xl font-bold mb-4 text-slate-700 dark:text-slate-200 flex items-center">
                    <span className="flex items-center justify-center w-8 h-8 mr-3 bg-sky-500 text-white rounded-full font-bold">2</span>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-center p-4">
                        <p className="text-slate-600 dark:text-slate-400 mb-6">Once your dates are selected, press the button to generate your expense report.</p>
                        <button
                            onClick={processExpenses}
                            disabled={loadingState.active || files.length === 0 || selectedDates.length === 0}
          ...
          <section id="step-3-summary">
            <div className="relative min-h-[200px] flex items-center justify-center">
              {loadingState.active && <Loader message={loadingState.message} progress={loadingState.progress} />}
              {error && (
              ...
              {!loadingState.active && !error && summary && <SummaryReport data={summary} />}
