...
// @ts-ignore - Vite worker query param type
import pdfWorkerSrc from 'pdfjs-dist/build/pdf.worker.mjs?worker&url';
import { TravelEntry } from '../types';
import { readFileAsBase64, readFileAsText, readFileAsArrayBuffer } from '../utils/fileUtils';
...
function rebuildPageTextWithLineBreaks(textContent: any): string {
  const items = (textContent?.items ?? []) as Array<{ str?: string; transform?: number[]; matrix?: number[] }>;
...
function parseJourneysHeuristically(text: string): TravelEntry[] {
  ...
  function toISO(y: string, m: string|number, d: string) {
    const mm = typeof m === 'number' ? m : monthMap[m.slice(0,3).toLowerCase()];
  ...
  for (const raw of lines) {
    ...
    if (/cap|capped|daily cap|weekly cap|total|payment|auto\s*top\s*up|auto\s*topup|refund|credit|adjustment/i.test(line)) {
      continue;
    }
...
function mergeEntriesByMaxCount(a: TravelEntry[], b: TravelEntry[]): TravelEntry[] {
  ...
  const ca = count(a);
  const cb = count(b);
  const keys = new Set<string>([...Array.from(ca.keys()), ...Array.from(cb.keys())]);
...
function parseCsvToTravelEntries(text: string): TravelEntry[] {
  ...
  const hasHeader = /(date|journey|day)/.test(header) && /(amount|total|charge|cost)/.test(header);
  ...
  function toISODate(s: string): string | null {
    ...
    m = s.match(/^(\d{1,2})[ \-](Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\w*[ \-](\d{4})$/i);
    if (m) {
      const monthMap: Record<string, number> = { jan:1,feb:2,mar:3,apr:4,may:5,jun:6,jul:7,aug:8,sep:9,sept:9,oct:10,nov:11,dec:12 };
      ...
      try {
        ...
        if (file.type === 'application/pdf') {
          ...
          for (let i = 1; i <= pdf.numPages; i++) {
            ...
            let pageText = rebuildPageTextWithLineBreaks(textContent);

            // If text is sparse, assume it's an image and use OCR
            if (pageText.trim().length < 100) { 
                onProgressUpdate(`Page ${i} appears image-based. Starting OCR...`);
                  ...
                  'eng',
                  {
                    logger: (m: any) => {
                      if (m.status === 'recognizing text') {
                        onProgressUpdate(`OCR on page ${i}: ${Math.round(m.progress * 100)}% complete`);
                      }
                    }
                  }
                );
          ...
          rawTextForFallback = combinedText;
          contents = { parts: [{ text: PROMPT }, { text: combinedText }] };
        ...
        try {
            ...
            throw new Error("The AI model returned a response in an unexpected format. Please try a different document or check the file quality.");
        }
        
        if (!parsedJson || !Array.isArray(parsedJson.expenses)) {
          console.warn("Gemini response was valid JSON but missing the 'expenses' array.", parsedJson);
          throw new Error("The AI model was unable to find any valid expense data in the document.");
        }
        ...
        return await traceFileProcessing(
          ...
          async () => {
            ...
            for (const item of parsedJson.expenses) {
              if (item && typeof item.date === 'string' && typeof item.amount === 'number' && !isNaN(item.amount)) {
            ...
            if (rawTextForFallback) {
              const heuristicEntries = parseJourneysHeuristically(rawTextForFallback);
              if (heuristicEntries.length) {
                finalEntries = mergeEntriesByMaxCount(validEntries, heuristicEntries);
              }
            }

            if (finalEntries.length === 0 && parsedJson.expenses.length > 0) {
              throw new Error("The AI model found some data, but none of it was in the correct format (e.g., YYYY-MM-DD for dates).");
            }

            onProgressUpdate(finalEntries.length === 0 
