...
const Calendar: React.FC<CalendarProps> = ({ selectedDates, onDateChange, currentDate, setCurrentDate }) => {
      ...
      <div key={day} className="flex items-center justify-center">
        <button
          ...
          className={`w-9 h-9 flex items-center justify-center text-sm rounded-full transition-all duration-200
            ${isSelected ? 'bg-sky-600 text-white font-semibold shadow-md hover:bg-sky-700' 
            : isToday ? 'bg-sky-100 dark:bg-sky-900/50 text-sky-700 dark:text-sky-200 font-semibold' 
