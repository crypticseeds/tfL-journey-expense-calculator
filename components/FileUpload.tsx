    ...
    <div className="w-full">
      <div
        className={`relative flex flex-col items-center justify-center w-full p-8 border-2 border-dashed rounded-xl transition-colors duration-300 ${
          isDragging ? 'border-sky-500 bg-sky-50 dark:bg-sky-900/20' : 'border-gray-300 dark:border-gray-600 bg-gray-50/50 dark:bg-gray-800/20'
      ...
      >
        <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Supports CSV, PDF, PNG, and JPG</p>
        </div>
        <input
          ref={fileInputRef}
          type="file"
      ...
      {selectedFiles.length > 0 && (
        <div className="mt-6">
            <div className="flex justify-between items-center mb-3">
                <h3 className="font-semibold text-gray-800 dark:text-gray-200">Uploaded Files</h3>
                {selectedFiles.length > 1 && (
                    <button
                        onClick={(e) => { e.stopPropagation(); if(!disabled) clearAllFiles();}}
            ...
            <div className="flex flex-wrap gap-3">
              {selectedFiles.map((file, index) => (
                <div key={index} className="flex items-center justify-between py-1.5 pl-3 pr-2 bg-slate-100 dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 text-sm font-medium text-slate-700 dark:text-slate-200 max-w-xs">
                  <div className="flex items-center min-w-0">
                    <FileIcon className="w-4 h-4 mr-2 text-slate-500 flex-shrink-0" />
                    <span className="truncate" title={file.name}>{file.name}</span>
                  </div>
                  <button
                    onClick={(e) => { e.stopPropagation(); if(!disabled) removeFile(index);}}
